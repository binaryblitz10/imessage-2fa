...
function parseAppleDate(dateStr: string): Date {
  try {
    ...
    return new Date(); // fallback to now if unparsable
  } catch {
    return new Date();
  }
}
...
async function getAppleMailMessages(
  ...
  const script = `
    tell application "Mail"
      try
        -- Robust mailbox selection across enabled accounts
        set inb to missing value
        repeat with acc in (every account whose enabled is true)
          try
            -- preferred common inbox names
            try
              set testBox to mailbox "INBOX" of acc
              set inb to testBox
              exit repeat
            end try
            try
            ...
            -- fallback: scan account mailboxes for any mailbox with messages
            try
              ...
              if inb is not missing value then exit repeat
            end try
          end try
        end repeat

        -- final fallback to unified inbox
        if inb is missing value then set inb to inbox
        ...
        repeat with i from 1 to 50
          try
            ...
            set output to output & msgId & "|||" & msgSubject & "|||" & msgSender & "|||" & htmlContent & "|||" & plainContent & "|||" & msgDateStr & "|||END|||"
          end try
        end repeat
        return output
      end try
  ...
  try {
    ...
    for (const rawMsg of rawMessages) {
      ...
      const date = parseAppleDate(dateStr);
      ...
      processedHtmlContent = processedHtmlContent.replace(/=([A-Fa-f0-9]{2})/g, (_, hex) => {
        try {
